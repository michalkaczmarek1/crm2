...
$(document).ready(function(){
	...
	var route = "http://pp42877.wsbpoz.solidhost.pl/";
	 ...
	 $(document).on('click', '#change-pass-salesman', function(e){
		 
		 e.preventDefault();
		 changePageTitle("Zmiana hasła - przedstawiciel");
			...
			form_html_change_pass += '<div class="col-md-4 col-md-offset-4">';
				...
				form_html_change_pass += "<a href='#' class='btn btn-primary btn-sm active dashboard-salesman'>powrót</a><br/>";
				form_html_change_pass += '</div>';
				form_html_change_pass += '</div>';
				form_html_change_pass += '</div>';
				form_html_change_pass += '</div>';
		...
		//	 obsługa formularza 
			...
			$(document).on("submit", "#change-pass-form", function(e){
				...
				$.ajax({
					url: route+"crm2.0/resources/users/salesman/password.php",
		    	    type : "POST",
